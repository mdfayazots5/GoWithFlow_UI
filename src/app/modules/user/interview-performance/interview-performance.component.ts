    ...
    <div class="min-h-screen bg-gw-bg">
      ...
      <div class="flex items-center gap-3">
        <div class="w-10 h-10 rounded-xl bg-gw-primary/10 flex items-center justify-center shrink-0">
          <i-lucide [img]="ChartIcon" size="20" class="text-gw-primary"></i-lucide>
        </div>
        <div>
      ...
      @else if (data()) {
        ...
        <div class="bg-white rounded-2xl border border-gw-card-border shadow-sm p-6">
          <div class="flex items-start justify-between">
            <div>
              <p class="text-[11px] font-black uppercase tracking-widest text-gw-text-muted italic">Interview Readiness Score</p>
              ...
              <p class="text-[11px] font-semibold text-gw-text-muted mt-1 italic">Based on last 5 sessions · {{ data()!.totalMockSessions }} total</p>
            </div>
            <div class="flex flex-col items-end gap-1">
              <div class="flex items-center gap-1 px-2.5 py-1.5 rounded-xl"
                ...
                <span class="text-[11px] font-black uppercase tracking-wider italic"
                  [class.text-gw-success]="data()!.readinessTrend === 'Improving'"
                  [class.text-amber-500]="data()!.readinessTrend === 'Stable'"
          ...
          <div class="flex justify-between mt-1">
            <span class="text-[11px] font-bold text-gw-text-muted italic">Needs Work</span>
            <span class="text-[11px] font-bold text-gw-text-muted italic">Interview Ready</span>
          </div>
        </div>
        ...
        @if (data()!.sessionTimeline.length > 0) {
          <div class="bg-white rounded-2xl border border-gw-card-border shadow-sm overflow-hidden">
            <div class="px-5 py-3 border-b border-gw-bg flex items-center gap-2">
              ...
              <p class="text-[11px] font-black uppercase tracking-widest text-gw-text-muted italic">Session Timeline</p>
            </div>
            <div class="px-5 py-4">
              ...
              <div class="flex items-center justify-between mt-1">
                <span class="text-[11px] font-bold text-gw-text-muted italic">Earliest</span>
                <span class="text-[11px] font-bold text-gw-text-muted italic">Latest</span>
              </div>
            </div>
            <div class="divide-y divide-gw-bg">
              @for (session of data()!.sessionTimeline.slice().reverse().slice(0, 5); track session.sessionId) {
                <div class="px-5 py-3 flex items-center justify-between gap-3">
                  <div>
                    <p class="text-[11px] font-bold text-gw-text-muted italic">{{ session.sessionDate | date:'MMM d, yyyy' }}</p>
                  ...
                  <div class="text-right">
                    ...
                    <p class="text-[11px] font-bold text-gw-text-muted italic">{{ session.mistakeCount }} errors</p>
                  </div>
                </div>
              }
            </div>
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div class="bg-white rounded-2xl border border-gw-card-border shadow-sm overflow-hidden">
            <div class="px-4 py-3 border-b border-gw-bg">
              <p class="text-[11px] font-black uppercase tracking-widest text-gw-text-muted italic">Grammar Errors</p>
            </div>
            <div class="divide-y divide-gw-bg">
              @if (data()!.topGrammarErrors.length === 0) {
                <p class="px-4 py-3 text-[11px] text-gw-text-muted italic">No errors recorded.</p>
              }
              @for (err of data()!.topGrammarErrors; track err.grammarTag) {
                <div class="px-4 py-2.5">
                  <p class="text-[11px] font-black text-gw-text italic truncate">{{ err.grammarTag }}</p>
                  <p class="text-[11px] font-bold text-gw-error mt-0.5">{{ err.errorCount }} errors</p>
                </div>
              }
            </div>
          </div>
          ...
          <div class="bg-white rounded-2xl border border-gw-card-border shadow-sm p-4 flex flex-col justify-between">
            <div>
              <p class="text-[11px] font-black uppercase tracking-widest text-gw-text-muted italic">Avg Answer Speed</p>
              <p class="text-3xl font-black text-gw-primary italic mt-1">{{ data()!.avgAnswerSpeedWpm | number:'1.0-0' }}</p>
              <p class="text-[11px] font-bold text-gw-text-muted italic">words per min</p>
            </div>
        ...
        @if (data()!.focusWordPerformance.length > 0) {
          <div class="bg-white rounded-2xl border border-gw-card-border shadow-sm overflow-hidden">
            <div class="px-5 py-3 border-b border-gw-bg flex items-center gap-2">
              ...
              <p class="text-[11px] font-black uppercase tracking-widest text-gw-text-muted italic">Professional Vocabulary</p>
            </div>
            <div class="divide-y divide-gw-bg">
              @for (word of data()!.focusWordPerformance; track word.focusWord) {
                <div class="px-5 py-3 flex items-center justify-between gap-3">
                  <div class="min-w-0">
                    ...
                    <p class="text-[11px] font-semibold text-gw-text-muted mt-0.5">{{ word.timesCorrect }} / {{ word.timesSpoken }} correct</p>
                  </div>
        ...
        @if (data()!.recommendedScriptId > 0) {
          <div class="bg-gw-primary/5 border border-gw-primary/20 rounded-2xl p-5">
            <div class="flex items-center gap-2 mb-2">
              ...
              <p class="text-[11px] font-black uppercase tracking-widest text-gw-primary italic">Recommended Next Session</p>
            </div>
            <p class="font-black text-gw-text italic">{{ data()!.recommendedScriptTitle }}</p>
            <p class="text-[11px] text-gw-text-muted italic mt-1">{{ data()!.recommendedScriptReason }}</p>
            <a [routerLink]="['/scripts']" [queryParams]="{ scriptId: data()!.recommendedScriptId }"
              class="mt-3 inline-block px-4 py-2 bg-gw-primary text-white font-black text-[11px] uppercase tracking-widest italic rounded-xl hover:opacity-90 transition-all">
              View Script
            </a>
          </div>
        }
